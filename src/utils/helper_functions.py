...
def load_profiles(directory):
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".csv"):
            ...
            ppe = filename.split("_")[-1].split(".")[
                0
            ]  # Wyciągnij nazwę PPE z nazwy pliku
            profiles[ppe] = df
    return profiles
...
def load_storages(filepath):
    ...
    if filepath.endswith(".json"):
        with open(filepath, "r") as f:
            ...
            for storage_id, storage_info in storage_data.items():
                storages.append(
                    {
                        "id": storage_id,
                        "capacity": storage_info["capacity"],
                        "current_charge": storage_info.get(
                            "current_charge", 0
                        ),  # Use 0 as default if not specified
                    }
                )
    else:
        # For backward compatibility with CSV
...
def plot_results(self, steps, labels, results_dir, formatted_date):
    ...
    plt.tight_layout()
    plt.savefig(
        results_dir / f"simulation_plots_{formatted_date}.png"
